...
async function supabaseFetch<T>(path: string): Promise<T[]> {
  ...
  try {
    ...
    console.error(`Supabase request failed: ${path}`, error);
    return [];
  }
}
...
const fallbackDbBanners: DbBanner[] = fallbackSlides.map((slide, index) => ({ id: String(index + 1), title: slide.title, subtitle: slide.description, image_url: slide.image, button_label: slide.cta, button_url: slide.href, sort_order: index + 1 }));
...
export async function getBanners(): Promise<DbBanner[]> {
  if (!hasSupabaseConfig) return fallbackDbBanners;
  const rows = await supabaseFetch<DbBanner & { starts_at?: string; ends_at?: string }>('homepage_banners?select=*&is_active=eq.true&order=sort_order.asc,created_at.desc');
  const now = Date.now();
