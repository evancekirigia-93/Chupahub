...
export default async function Product({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ variant?: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8">
    ...
    <section className="grid gap-8 rounded-3xl bg-white p-4 shadow-card md:grid-cols-2 md:p-6"><ProductGallery images={images} name={`${product.name} – ${categoryName} delivery Nairobi`} /><div><p className="font-bold uppercase tracking-wide text-brand-orange">{product.brands?.name || 'ChupaHub selection'}</p><h1 className="mt-2 text-3xl font-black text-brand-ink sm:text-4xl">{product.name}</h1><p className={`mt-3 inline-flex rounded-full px-3 py-1 text-sm font-bold ${availability ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>{availability ? 'Available' : 'Out of stock'}</p><ProductPurchase product={product} initialVariantId={initialVariantId} /><dl className="mt-6 grid grid-cols-2 gap-3 rounded-2xl bg-brand-soft p-4 text-sm"><dt className="font-bold">ABV</dt><dd>{product.abv != null ? `${product.abv}%` : 'Not specified'}</dd><dt className="font-bold">Country</dt><dd>{product.country || 'Not specified'}</dd><dt className="font-bold">Sizes</dt><dd>{product.product_variants?.length ? 'Select a size above' : product.bottle_size || 'Not specified'}</dd><dt className="font-bold">Availability</dt><dd>{availability ? 'Available' : 'Out of stock'}</dd></dl></div></section>
