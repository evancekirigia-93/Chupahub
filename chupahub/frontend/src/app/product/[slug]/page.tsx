...
export default async function Product({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="mx-auto max-w-none px-4 py-8">
    ...
    <section className="grid gap-8 rounded-3xl bg-white p-5 shadow-card md:grid-cols-2 md:p-8"><div className="grid gap-3 sm:grid-cols-2">{images.map((image) => <div key={image} className="flex h-[clamp(28rem,48vw,37.5rem)] items-center justify-center rounded-2xl bg-white p-6"><img src={image} alt={`${product.name} – ${categoryName} delivery Nairobi`} className="h-[90%] w-[90%] object-contain object-center" /></div>)}</div><div><p className="font-bold uppercase tracking-wide text-brand-orange">{product.brands?.name}</p><h1 className="mt-2 text-4xl font-black text-brand-ink sm:text-5xl">{product.name}</h1><div className="mt-4 text-neutral-600" dangerouslySetInnerHTML={{ __html: description }} /><dl className="mt-6 grid grid-cols-2 gap-3 rounded-2xl bg-brand-soft p-4 text-sm"><dt className="font-bold">ABV</dt><dd>{product.abv}%</dd><dt className="font-bold">Country</dt><dd>{product.country}</dd><dt className="font-bold">Bottle Size</dt><dd>{product.bottle_size}</dd><dt className="font-bold">Stock</dt><dd>{product.stock}</dd></dl><ProductPurchase product={product} /></div></section><ProductRail title="Customers also bought" products={related.slice(0, 8)} />
  </main>;
}
