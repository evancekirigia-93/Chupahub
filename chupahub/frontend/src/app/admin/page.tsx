...
async function rest(table: string, token: string, init?: RequestInit) {
  const res = await fetch(`${supabaseUrl}/rest/v1/${table}${init?.method === 'POST' ? '' : '?select=*&order=created_at.desc&limit=50'}`, { ...init, headers: { apikey: supabaseAnonKey, Authorization: `Bearer ${token}`, 'Content-Type': 'application/json', Prefer: 'return=representation', ...(init?.headers || {}) } });
...
export default function Admin() {
  ...
  const [form, setForm] = useState<Row>({ name: '', slug: '', price: 0, is_active: true });

  async function load() { if (!session) return; setRows(await rest(table, session.access_token)); }
  useEffect(() => { load().catch((e) => setError(e.message)); }, [session, table]);
  ...
  async function save(e: FormEvent) { e.preventDefault(); if (!session) return; const payload = Object.fromEntries(Object.entries(form).filter(([k, v]) => k !== 'id' && v !== '' && v !== null)); const target = form.id ? `${table}?id=eq.${form.id}` : table; await rest(target, session.access_token, { method: form.id ? 'PATCH' : 'POST', body: JSON.stringify(payload) }); setForm({ name: '', slug: '', price: 0, is_active: true }); await load(); }
  ...
  return <main className="mx-auto max-w-none px-4 py-8"><div className="rounded-3xl bg-white p-6 shadow-card"><p className="font-bold uppercase tracking-wide text-brand-orange">Supabase Admin</p><h1 className="text-4xl font-black text-brand-ink">ChupaHub Admin Dashboard</h1><p className="mt-2 text-neutral-600">CRUD changes write directly to Supabase and are visible on Vercel after the 30-second page revalidation window.</p></div><div className="mt-6 grid gap-6 lg:grid-cols-[260px_1fr]"><aside className="rounded-2xl bg-white p-4 shadow-card">{tables.map((t) => <button key={t} onClick={() => setTable(t)} className={`mb-2 block w-full rounded-xl px-3 py-2 text-left font-bold ${table === t ? 'bg-brand-deep text-white' : 'bg-brand-soft text-brand-ink'}`}>{t}</button>)}</aside><section className="rounded-2xl bg-white p-4 shadow-card"><h2 className="text-2xl font-black capitalize">{table}</h2><form onSubmit={save} className="my-4 grid gap-3 md:grid-cols-4"><input placeholder="name/title" value={form.name || form.title || ''} onChange={(e) => setForm({ ...form, name: e.target.value, title: e.target.value })} className="rounded-xl border p-3" /><input placeholder="slug/code" value={form.slug || form.code || ''} onChange={(e) => setForm({ ...form, slug: e.target.value, code: e.target.value })} className="rounded-xl border p-3" /><input placeholder="price" type="number" value={form.price || ''} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} className="rounded-xl border p-3" /><button className="rounded-xl bg-brand-deep font-black text-white">{form.id ? 'Update' : 'Create'}</button></form>{error && <p className="text-red-600">{error}</p>}<div className="overflow-auto"><table className="w-full text-sm"><thead><tr className="text-left"><th className="p-2">Name</th><th className="p-2">Slug/Status</th><th className="p-2">Actions</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t"><td className="p-2 font-bold">{row.name || row.title || row.email || row.id}</td><td className="p-2">{row.slug || row.status || row.code || String(row.is_active)}</td><td className="flex gap-2 p-2"><button onClick={() => setForm(row)} className="rounded-lg bg-brand-orange px-3 py-1 font-bold text-white">Edit</button><button onClick={() => remove(row.id)} className="rounded-lg bg-red-600 px-3 py-1 font-bold text-white">Delete</button></td></tr>)}</tbody></table></div></section></div></main>;
}
