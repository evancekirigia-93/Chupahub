...
export function Header({ content = {} }: { content?: { header_notice?: string; logo_text?: string } }) {
  ...
  return <>
    <header className="text-white shadow-orange">
      <div className="bg-white text-brand-ink"><div className="mx-auto flex max-w-none items-center justify-between px-3 py-1 text-[10px] font-semibold sm:px-6"><span>{content.header_notice || 'Delivery within Nairobi: 10-50min'}</span><span className="rounded-full bg-brand-ink px-2 py-0.5 text-white">18+</span></div></div>
      <nav className="bg-brand-deep"><div className="mx-auto max-w-none px-3 py-2 sm:px-6"><div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3"><Link href="/" className="order-1 flex items-center focus-ring" aria-label="ChupaHub home"><img src="/chupahub-logo.svg" alt="" className="h-12 w-11 rounded-md object-contain shadow-sm sm:h-14 sm:w-12"/><span className="ml-2 text-xl font-black tracking-tight text-white sm:text-2xl">{content.logo_text || 'ChupaHub'}</span></Link><button className="order-2 flex items-center gap-2 rounded-xl px-1 py-2 text-sm uppercase tracking-wide focus-ring" aria-label="Open menu"><Menu size={28} /><span className="hidden sm:inline">Menu</span></button><div className="order-3 ml-auto flex items-center gap-2 sm:gap-3"><Link className="focus-ring" href="/account" aria-label="Account"><UserCircle size={30} /></Link><Heart className="hidden sm:block" aria-label="Wishlist" /><button onClick={() => setCartOpen(true)} className="relative rounded-xl border-2 border-white px-3 py-2 focus-ring" aria-label="Open cart"><ShoppingBag /><span className="absolute -right-2 -top-2 grid h-6 place-items-center rounded-full bg-white text-xs font-black text-brand-deep">{cartCount}</span></button></div><label className="order-4 mt-1 flex min-w-0 flex-1 basis-full items-center gap-2 rounded-full bg-white px-3 py-2 text-brand-ink shadow-card sm:px-4"><Search className="text-brand-orange" /><input className="w-full bg-transparent text-sm outline-none placeholder:text-neutral-500" placeholder="Search products..." aria-label="Search products" /></label></div></div></nav>
    </header>
