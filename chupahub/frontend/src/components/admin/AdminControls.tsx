...
export function ImageDropzone({ busy, onFiles }: { busy: boolean; onFiles: (files: File[]) => void }) {
  ...
  return <label onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop} className={`flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-4 text-center transition ${dragging ? 'border-brand-orange bg-orange-50' : 'border-orange-200 bg-brand-soft/50'}`}><input type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif" disabled={busy} onChange={(event) => accept(event.target.files)} className="sr-only" />{busy ? <Loader2 className="animate-spin text-brand-orange" /> : <UploadCloud className="text-brand-orange" />}<span className="mt-2 text-sm font-bold">Drop multiple images or click to browse</span><span className="text-xs text-neutral-500">JPG, PNG, WebP or GIF · max 10 MB each</span></label>;
}
