...
const FROM = 'ChupaHub <orders@chupahub.com>';
...
export async function sendOrderEmail(db: SupabaseClient, order: EmailOrder, event: OrderEmailEvent, recipient: string) {
  ...
  const copy = eventCopy[event];
  const result = await resend(recipient, copy.subject(order.orderNumber), orderEmailHtml(order, event));
