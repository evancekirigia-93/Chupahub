...
export function HeroCarousel({ banners }: { banners: DbBanner[] }) {
  ...
  return <section onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onTouchStart={() => setPaused(true)} onTouchEnd={() => setPaused(false)} aria-roledescription="carousel" aria-label="ChupaHub promotions" className="relative mx-auto max-w-none overflow-hidden bg-white shadow-card sm:mt-4 sm:rounded-3xl">
    <div className="relative h-56 sm:h-96">{slides.map((banner,index) => {
      const className = `absolute inset-0 transition-opacity duration-500 ${index === current ? 'z-10 opacity-100' : 'pointer-events-none opacity-0'}`;
      return banner.button_url
        ? <Link key={banner.id} href={banner.button_url} aria-hidden={index !== current} tabIndex={index === current ? 0 : -1} aria-label={`${banner.title}: ${banner.button_label || banner.button_text || 'Open offer'}`} className={className}><SlideContent banner={banner}/></Link>
        : <article key={banner.id} aria-hidden={index !== current} className={className}><SlideContent banner={banner}/></article>;
    })}</div>
    {slides.length > 1 && <><button type="button" onClick={()=>move(-1)} aria-label="Previous hero image" className="absolute left-3 top-1/2 z-20 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-brand-ink shadow-card"><ChevronLeft/></button><button type="button" onClick={()=>move(1)} aria-label="Next hero image" className="absolute right-3 top-1/2 z-20 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-brand-ink shadow-card"><ChevronRight/></button><div className="absolute bottom-3 right-4 z-20 flex gap-2">{slides.map((banner,index)=><button key={banner.id} onClick={()=>setCurrent(index)} aria-label={`Show hero image ${index+1}`} className={`h-2.5 rounded-full transition-all ${index===current?'w-7 bg-white':'w-2.5 bg-white/55'}`}/>)}</div></>}
  </section>;
}
