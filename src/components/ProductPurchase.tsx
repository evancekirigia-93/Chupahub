...
export function ProductPurchase({ product, initialVariantId }: { product: DbProduct; initialVariantId?: string }) {
  ...
  function addToCart() {
    ...
    writeCart(cart, { item: addedItem, quantityAdded: quantity });
  }
  ...
  return <div className="mt-4">
    ...
    <div className="mt-4 flex items-end justify-between gap-4"><div><p className="text-3xl font-black text-brand-deep">{money(price)}</p>{pricing.oldPrice && <p className="mt-1 text-sm text-neutral-500 line-through">{money(pricing.oldPrice)}</p>}</div><p className={`text-sm font-bold ${available ? 'text-green-700' : 'text-red-600'}`}>{available ? (maxQuantity <= (selected?.low_stock_threshold || product.low_stock_threshold || 5) ? 'Low stock' : 'Available now') : 'Currently unavailable'}</p></div>
