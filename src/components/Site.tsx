...
export function ProductRail({ title, products, href }: { title: string; products: DbProduct[]; href: string; limit?: number }) {
  ...
  return <section className="product-rail relative mx-auto max-w-none overflow-hidden px-2 py-7 sm:px-3 sm:py-9">
    ...
    <button type="button" onClick={() => scrollProducts(-1)} aria-label={`Scroll ${title} products left`} className="product-rail-arrow product-rail-arrow-left">‹</button>
    <button type="button" onClick={() => scrollProducts(1)} aria-label={`Scroll ${title} products right`} className="product-rail-arrow product-rail-arrow-right">›</button>
  </section>;
}
