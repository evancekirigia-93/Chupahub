...
export function CategoryGrid({ categories }: { categories: DbCategory[] }) {
  return <section className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-3 py-5 sm:grid-cols-3 sm:px-4 md:grid-cols-4 lg:grid-cols-6">{categories.map((category) => <Link href={`/category/${category.slug}`} key={category.id} className="group relative aspect-[4/3] overflow-hidden rounded-xl bg-neutral-100 shadow-sm ring-1 ring-black/5 transition hover:-translate-y-0.5 hover:shadow-card"><SmartImage src={category.image_url || 'https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?auto=format&fit=crop&w=700&q=80'} alt={`${category.name} category`} sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 17vw" className="transition duration-500 group-hover:scale-[1.03]" /><div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-black/75 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-2.5 text-white"><h2 className="truncate text-xs font-semibold tracking-wide sm:text-sm">{category.name}</h2></div></Link>)}</section>;
}
