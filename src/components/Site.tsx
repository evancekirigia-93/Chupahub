...
export function Header({ content = {}, products = [] }: { content?: SiteContent; products?: DbProduct[] }) {
  ...
  const categories = [['All Products','/shop'],['Beer','/beer'],['Wine','/wine'],['Whisky','/whisky'],['Vodka','/vodka'],['Gin','/gin'],['Champagne','/champagne'],['Brandy','/brandy'],['Rum','/rum'],['Tequila','/tequila'],['Mixers','/mixers'],['Soft Drinks','/soft-drinks'],['Energy Drinks','/energy-drinks'],['Offers','/offers'],['Same-Day Delivery','/same-day-delivery'],['Alcohol Delivery Nairobi','/alcohol-delivery-nairobi'],['Search','/search'],['Contact','/contact'],['About','/about'],['FAQ','/faq'],['Track Order','/track-order']];
  ...
  const menuLinks = () => <div className="grid grid-cols-2 gap-2">{categories.map(([label,href])=><Link key={label} onClick={()=>setMenu(false)} href={href} className="rounded-xl border border-orange-100 bg-orange-50 px-3 py-2.5 text-sm font-bold text-brand-ink transition hover:border-brand-orange hover:bg-orange-100 focus:outline-none focus:ring-2 focus:ring-brand-orange">{label}</Link>)}<Link href="/account" onClick={()=>setMenu(false)} className="rounded-xl border border-orange-100 bg-orange-50 px-3 py-2.5 text-sm font-bold text-brand-ink">Account</Link><Link href="/wishlist" onClick={()=>setMenu(false)} className="rounded-xl border border-orange-100 bg-orange-50 px-3 py-2.5 text-sm font-bold text-brand-ink">Wishlist</Link><a href="https://wa.me/" onClick={()=>setMenu(false)} className="rounded-xl border border-orange-100 bg-orange-50 px-3 py-2.5 text-sm font-bold text-brand-ink">Contact us</a></div>;
  return <header className="brand-gradient sticky top-0 z-40">
    ...
    <div className="relative flex w-full flex-wrap items-center gap-3 px-3 py-3 sm:px-5">
      <Link href="/" className="flex shrink-0 items-center" aria-label="ChupaHub home"><BrandLogo logoUrl={content.logo_url} /></Link>
      <div className="relative hidden md:block">
        <button type="button" onClick={() => setMenu(!menu)} aria-expanded={menu} aria-haspopup="menu" className="flex items-center gap-1 rounded-xl border border-white/50 bg-white px-3 py-2 text-sm font-bold text-brand-ink"><Menu size={20}/> Categories</button>
        {menu && <nav aria-label="Product categories" className="absolute left-0 top-full z-[70] mt-2 max-h-[min(70vh,38rem)] w-[24rem] overflow-y-auto rounded-2xl border border-orange-100 bg-white p-3 shadow-2xl">{menuLinks()}</nav>}
      </div>
      <Link href="/checkout" className="hidden max-w-40 truncate rounded-xl bg-orange-50 px-3 py-2 text-xs font-bold text-brand-ink lg:block"><MapPin className="mr-1 inline text-brand-orange" size={15}/>{location}</Link>
      <div className="relative order-4 w-full md:order-none md:min-w-64 md:flex-1"><Search className="absolute left-3 top-3 text-brand-orange" size={20}/><input value={query} onChange={e=>setQuery(e.target.value)} className="w-full rounded-xl border-2 border-orange-100 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-brand-orange" placeholder="Search products, brands and categories" aria-label="Search products"/>{suggestions.length > 0 && <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-xl border border-orange-100 bg-white shadow-card">{suggestions.map(p => <Link key={p.id} href={`/product/${p.slug}`} onClick={()=>setQuery('')} className="flex items-center gap-3 p-3 hover:bg-orange-50"><img src={imageFor(p)} alt={`${p.name} product`} loading="lazy" decoding="async" className="h-10 w-10 object-contain"/><span className="min-w-0 flex-1"><b className="block truncate">{p.name}</b><small>{p.bottle_size || p.product_variants?.[0]?.name || 'Select size'} · {money(p.product_variants?.[0]?.price ?? p.price)}</small></span></Link>)}<Link href={`/search?q=${encodeURIComponent(query)}`} className="block border-t border-orange-100 p-3 text-sm font-black text-brand-orange">View all results</Link></div>}</div>
      <div className="ml-auto flex items-center gap-3 text-white"><a href="https://wa.me/" aria-label="Contact ChupaHub on WhatsApp" className="hidden sm:block"><MessageCircle/></a><Link href="/account" aria-label="Account"><UserCircle/></Link><Link href="/wishlist" className="relative" aria-label="Wishlist"><Heart/><span className="absolute -right-2 -top-2 rounded-full bg-brand-ink px-1.5 text-[10px] font-black text-white">0</span></Link><Link href="/checkout" data-cart-icon className="relative flex items-center gap-1" aria-label="Cart"><ShoppingBag/><span className="absolute -right-2 -top-2 rounded-full bg-brand-ink px-1.5 text-[10px] font-black text-white">{cart.count}</span><span className="hidden text-xs font-black lg:inline">{money(cart.total)}</span></Link><button type="button" onClick={() => setMenu(!menu)} aria-expanded={menu} aria-label="Open categories" className="md:hidden"><Menu/></button></div>
      {menu && <nav aria-label="Product categories" className="absolute left-3 right-3 top-full z-[70] mt-2 max-h-[70vh] overflow-y-auto rounded-2xl border border-orange-100 bg-white p-3 shadow-2xl md:hidden">{menuLinks()}</nav>}
    </div>
  </header>;
}
