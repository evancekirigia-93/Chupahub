...
export function Header({ content = {}, products = [] }: { content?: SiteContent; products?: DbProduct[] }) {
  ...
  return <header className="brand-gradient sticky top-0 z-40">
    ...
    <div className="flex w-full flex-wrap items-center gap-3 px-3 py-3 sm:px-5">
      <Link href="/" className="flex shrink-0 items-center" aria-label="ChupaHub home"><BrandLogo src={content.logo_url} /></Link>
    ...
    <nav aria-label="Primary navigation" className="flex w-full gap-2 overflow-x-auto border-t border-white/15 px-3 py-2.5 [scrollbar-width:none] sm:px-5 [&::-webkit-scrollbar]:hidden">{primaryLinks.map(([label,href]) => <Link key={href} href={href} className="shrink-0 rounded-full border border-white/30 bg-white/95 px-4 py-2 text-sm font-bold text-brand-ink shadow-sm transition hover:bg-orange-50">{label}</Link>)}</nav>
  </header>;
}
