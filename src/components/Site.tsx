...
import { Heart, MapPin as MapPinIcon, Menu as MenuIcon, MessageCircle, Search, ShoppingBag, UserCircle } from 'lucide-react';
...
export function CategoryGrid({ categories }: { categories: DbCategory[] }) {
  return <section className="mx-auto grid max-w-none grid-cols-3 gap-2 px-3 py-4 sm:grid-cols-4 sm:gap-3 sm:px-4 md:grid-cols-6 xl:grid-cols-8">{categories.map((category) => <Link href={`/category/${category.slug}`} key={category.id} className="group relative aspect-square overflow-hidden rounded-2xl bg-neutral-100 shadow-card"><SmartImage src={category.image_url || 'https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?auto=format&fit=crop&w=700&q=80'} alt={`${category.name} category`} sizes="(max-width: 640px) 33vw, (max-width: 1024px) 17vw, 13vw" className="transition duration-500 group-hover:scale-105" /><div className="tile-shade absolute inset-0" /><div className="absolute inset-x-0 bottom-0 p-2 text-center text-white"><h2 className="text-sm font-bold sm:text-base">{category.name}</h2></div></Link>)}</section>;
}
