...
export default async function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (collection) {
    ...
    products = section.product_ids?.length
      ? section.product_ids.map(id => allProducts.find(product => product.id === id)).filter((product): product is typeof allProducts[number] => Boolean(product))
      : section.use_best_sellers
        ? allProducts.filter(product => product.is_top_seller)
        : section.categories?.slug
          ? allProducts.filter(product => product.categories?.slug === section.categories?.slug)
          : allProducts;
  }
