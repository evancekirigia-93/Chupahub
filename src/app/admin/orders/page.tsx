...
const paymentLabel = (value: string) => value === 'paid' ? 'Paid' : 'Unpaid';
...
export default function OrdersPage() {
  ...
  async function changePayment(order: Order, paymentStatus: 'paid' | 'unpaid') {
    if (!supabase) return; setNotice(''); setError('');
    const { data: { session } } = await supabase.auth.getSession();
    const response = await fetch(`/api/admin/orders/${order.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ paymentStatus }) });
    const result = await response.json();
    if (!response.ok) { setError(result.error || 'Unable to update payment.'); return; }
    const updated = await fetchOrder(order.id); if (updated) mergeOrder(updated);
    setNotice(`Order ${order.order_number || order.id} marked ${paymentLabel(paymentStatus)}.`); window.setTimeout(() => setNotice(''), 4000);
  }

  const filtered = orders.filter(order => {
    const matchesStatus = !status || (status === 'paid' ? order.payment_status === 'paid' : order.payment_status !== 'paid');
  ...
  return <main className="mx-auto max-w-7xl p-3 sm:p-5">
    ...
    {alertOrder && <aside role="alert" className="fixed inset-x-3 bottom-3 z-50 rounded-2xl border-2 border-brand-orange bg-white p-4 shadow-2xl sm:inset-x-auto sm:bottom-auto sm:right-5 sm:top-5 sm:w-96"><button onClick={() => acknowledge(alertOrder.id)} className="absolute right-3 top-3" aria-label="Acknowledge new order"><X/></button><div className="flex items-center gap-2 text-brand-orange"><Bell/><b>New order received</b><Volume2 size={16}/></div><p className="mt-2 font-black">#{alertOrder.order_number || alertOrder.id.slice(0, 8)}</p><p>{alertOrder.customer_name || 'Guest'} · {money(alertOrder.total)}</p><div className="mt-3 flex gap-2"><Link href="/admin/dispatch" className="rounded-xl bg-brand-orange px-4 py-2 font-black text-white">Open dispatch</Link><button onClick={() => acknowledge(alertOrder.id)} className="rounded-xl border px-4 py-2 font-bold">Acknowledge</button></div></aside>}
    <div className="mt-5 flex flex-wrap gap-3"><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search order, customer or phone" className="min-w-64 flex-1 rounded-xl border bg-white p-3"/><select value={status} onChange={event => setStatus(event.target.value)} className="rounded-xl border bg-white p-3"><option value="">All payments</option>{paymentOptions.map(value => <option key={value} value={value}>{paymentLabel(value)}</option>)}</select></div>
    {loading ? <p className="mt-8">Loading orders…</p> : error ? <div className="mt-8 rounded-xl bg-red-50 p-4 text-red-700"><b>Orders could not be refreshed.</b><p>{error}</p><button onClick={() => void load(true)} className="mt-3 rounded-lg border border-red-300 px-3 py-2 font-bold">Try again</button></div> : !visible.length ? <p className="mt-8 rounded-xl bg-white p-6 shadow-card">No orders match these filters.</p> : <div className="mt-5 overflow-x-auto rounded-2xl bg-white shadow-card"><table className="w-full text-left text-sm"><thead className="bg-brand-soft"><tr>{['Order','Date','Customer','Phone','Address','Products','Payment','Delivery','Total','Dispatch','Confirm payment'].map(label => <th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{visible.map(order => <tr key={order.id} className={`border-t transition-colors duration-700 ${highlighted.has(order.id) ? 'bg-orange-100' : ''}`}><td className="p-3 font-bold"><Link className="text-brand-orange" href={`/admin/orders/${order.id}`}>{order.order_number || order.id.slice(0,8)}</Link></td><td className="whitespace-nowrap p-3">{new Date(order.created_at).toLocaleString()}</td><td className="p-3">{order.customer_name || 'Guest'}</td><td className="p-3"><a href={`tel:${order.customer_phone || ''}`}>{order.customer_phone || '—'}</a></td><td className="max-w-48 truncate p-3">{order.delivery_address || '—'}{order.delivery_location_verified === false && <small className="block font-bold text-red-600">Location not verified</small>}</td><td className="p-3">{order.order_items?.[0]?.count || 0}</td><td className="p-3">{order.payment_method}<br/><small>{order.payment_status}</small></td><td className="p-3">{money(order.delivery_fee)}</td><td className="p-3 font-bold">{money(order.total)}</td><td className="p-3"><span className={`rounded-full px-2 py-1 text-xs font-black capitalize ${statusClass(order.status)}`}>{statusLabel(order.status)}</span></td><td className="p-3"><select aria-label={`Confirm payment for order ${order.order_number || order.id}`} value={order.payment_status === 'paid' ? 'paid' : 'unpaid'} onChange={event => void changePayment(order, event.target.value as 'paid' | 'unpaid')} className={`rounded-lg border p-2 font-bold ${order.payment_status === 'paid' ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-800'}`}><option value="paid">Paid</option><option value="unpaid">Unpaid</option></select></td></tr>)}</tbody></table></div>}
