...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = truncate(product.seo_description || plainText(product.short_description || product.description) || `Order ${product.name} online from Chupa Hub for fast alcohol delivery across Nairobi.`);
  ...
  return {
    ...
    openGraph: { title: `${title} | Chupa Hub`, description, url, type: 'website', images: images.map((image) => ({ url: image, alt: product.name })) },
    twitter: { card: 'summary_large_image', title: `${title} | Chupa Hub`, description, images: images.slice(0, 1) },
  };
}
