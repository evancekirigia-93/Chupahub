...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (emailEvent && order.customer_email) {
    const emailOrder: EmailOrder = { id: order.id, orderNumber: order.order_number, customerName: order.customer_name || 'Customer', customerEmail: order.customer_email, customerPhone: order.customer_phone, deliveryAddress: order.delivery_address || 'Delivery address on order', paymentMethod: order.payment_method, subtotal: Number(order.subtotal), deliveryFee: Number(order.delivery_fee), total: Number(order.total), estimatedDelivery: body.status === 'delivered' ? 'Delivered' : body.status === 'cancelled' || body.status === 'rejected' ? 'Cancelled' : '10–50 minutes', riderName: body.riderName || order.rider_name, riderPhone: body.riderPhone || order.rider_phone, dispatchTime: body.status === 'dispatched' ? new Date().toLocaleString('en-KE', { timeZone: 'Africa/Nairobi' }) : null, items: (order.order_items || []).map(item => ({ name: item.product_name, quantity: Number(item.quantity), unitPrice: Number(item.unit_price), lineTotal: Number(item.line_total) })) };
