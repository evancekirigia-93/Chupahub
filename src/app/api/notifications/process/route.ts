...
async function processOutbox(request:NextRequest){
  ...
  const {data:deliveries,error}=await db.from('notification_deliveries')
    .select('id,order_id,recipient,event_key,attempts').eq('channel','email').in('status',['pending','failed']).lt('attempts',3).order('created_at').limit(20);
  ...
  return Response.json({processed:(deliveries||[]).length,sent,failed});
}
